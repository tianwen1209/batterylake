...
(function () {
  ...
  var FAQ = [
    ...
    {
      id: 'contribute',
      keys: ['contribute', 'contribution', 'submit a dataset', 'submit my dataset', 'share my data', 'upload my dataset', 'add a dataset', 'intake', 'required files', 'submission package', '贡献', '提交数据集', '上传我的数据', '分享数据', '添加数据集', '需要哪些文件', '投稿'],
      en: 'Use the [Contribute page](#contribute); it walks through four steps in your browser, nothing is uploaded:\n- **Describe** — dataset name, lab, year, chemistry, form factor, cells, capacity, C-rates, temperature, DOI, license, data link and protocol notes. The reference name (naming standard) and `metadata.json` are generated live.\n- **Check** — drop a small CSV/JSON excerpt: the quality engine detects the channels (voltage, current, timestamp, temperature, capacity, cell id) and scores it; a readiness checklist shows what is still missing.\n- **Package** — download a zip with metadata.json, protocol.md, README, checklist and the pre-check.\n- **Submit** — open a prefilled GitHub issue in BatteryLake-Benchmark-DataPrep with the summary and attach the package.\n\nThe team then runs the batterylake-processing skill (five gates) on your files and publishes the dataset with your DOI and license. In Agent mode you can say, for example, "I want to contribute 24 NMC 21700 cells from NTU, 2025, 1C/1C at 25 °C" and the form is filled for you.',
      zh: '请使用 [Contribute 页面](#contribute)，四步都在浏览器里完成，不会上传任何数据：\n- **Describe 描述** — 数据集名称、机构、年份、化学体系、形态、电芯数、容量、倍率、温度、DOI、许可、数据下载链接和协议说明。标准名（命名规范）和 `metadata.json` 实时生成。\n- **Check 检查** — 拖入一小段 CSV/JSON 样本：质量引擎识别通道（电压、电流、时间戳、温度、容量、电芯 id）并打分；就绪清单显示还缺什么。\n- **Package 打包** — 下载包含 metadata.json、protocol.md、README、清单和预检结果的 zip。\n- **Submit 提交** — 打开预填好的 GitHub issue（BatteryLake-Benchmark-DataPrep 仓库），附上压缩包。\n\n之后团队用 batterylake-processing skill（五个 gate）处理你的文件，并以你的 DOI 和许可发布。在 Agent 模式下可以直接说“我想贡献一个数据集：24 颗 NMC 21700 电芯，来自 NTU，2025 年，1C/1C，25 度”，表单会自动填好。'
    },
    {
      id: 'cite',
