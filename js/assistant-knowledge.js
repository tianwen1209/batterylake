...
(function () {
  ...
  var FAQ = [
    ...
    {
      id: 'contribute',
      keys: ['contribute', 'contribution', 'submit a dataset', 'submit my dataset', 'share my data', 'upload my dataset', 'add a dataset', 'intake', 'required files', '贡献', '提交数据集', '上传我的数据', '分享数据', '添加数据集', '需要哪些文件'],
      en: 'Use the [Contribute page](#contribute). A submission package needs:\n- `raw_cycles/` — native cycler exports or normalized CSV/Parquet time series (voltage, current, timestamp, cell id).\n- `metadata.json` — chemistry, nominal capacity, protocol, institution, DOI, license.\n- `protocol.md` — charge/discharge details, rest periods, C-rates, formation cycles, cutoffs, equipment.\n- attribution (DOI / repository / license) for the source.\nEvery contribution goes through the same ETL, five-gate processing and quality assessment as the curated datasets, and keeps attribution to the original authors.',
      zh: '请使用 [Contribute 页面](#contribute)。提交包需要：\n- `raw_cycles/` — 原生测试仪导出或规范化的 CSV/Parquet 时序（电压、电流、时间戳、电芯 id）。\n- `metadata.json` — 化学体系、标称容量、协议、机构、DOI、许可。\n- `protocol.md` — 充放电细节、静置、倍率、化成循环、截止条件、设备。\n- 来源署名（DOI / 仓库 / 许可）。\n所有贡献都会经过与现有数据集相同的 ETL、五 gate 处理和质量评估，并保留对原作者的署名。'
    },
    {
      id: 'cite',
