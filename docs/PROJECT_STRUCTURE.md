...
- `js/contribute.js`: Contribute page (Describe → Check → Package → Submit): live ref_name / metadata.json from the form, browser quality pre-check of a data sample, optional status.json gate track, readiness checklist, in-browser submission zip, prefilled GitHub issue link; exposes `window.BatteryLakeContribute` for the AI agent. Styles in `styles/contribute.css`.
