...
- `js/assistant-actions.js`: Agent-mode tool catalog (whitelisted page actions), executor, planner prompt and rule-based fallback planner.
...
- `js/contribute.js`: Contribute page (Describe → Upload raw data → Submit): live ref_name / metadata.json from the form, chunked resumable upload of raw files to the configured upload endpoint (Worker + R2 or app.py), quick quality check of the first small CSV, optional status.json gate track, readiness checklist, submission.json stored next to the files, prefilled GitHub issue; exposes `window.BatteryLakeContribute` for the AI agent. Styles in `styles/contribute.css`.
- `js/quality-engine.js`: browser port of `quality/quality_assessment.py` used by the Quality Assessment page (per-cell checks, v2 canonical column names, `metrics` for the card footers). Precomputed reports live in `quality_reports/` (generated by `quality/run_quality_v2.py` from the canonical v2 time series); the page's "try it" CSV excerpts live in `assets/examples/quality/`.
...
- `js/assistant-actions.js` (Agent-mode tools)
